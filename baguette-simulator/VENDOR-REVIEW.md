...
Native helper update: releaseKeys now accumulates delivery failures instead of returning on the first failure, attempting HID usages 4–231 before reporting aggregate success. Rebuilt with `sh native/build.sh` and verified ad-hoc signature. Updated native/keyboard SHA-256: `2ee3946de04ebb35e057b459248a41bbf8b2d861a1ab70b1d43e8e5ceb5c9517`. The compiled C regression executes the production release function with first/middle/last/all delivery failures. Existing per-delivery and process timeouts still bound unresponsive transport. Upstream Baguette and vendor files are unchanged.
