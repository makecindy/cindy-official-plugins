...
function render() {
  if (!state) return;
  for (const [id,list] of [['interact-list',state.interact.allow],['read-list',state.read.block],['exclusions-list',state.interact.block]]) {
  ...
  $('#exclusions-label').hidden = !state.interact.block.length;
