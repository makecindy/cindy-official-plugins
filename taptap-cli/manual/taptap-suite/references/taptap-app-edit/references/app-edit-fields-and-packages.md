...
- 每条 change 先读取最新并发快照：字段返回 `expected_digest` 时原样传 `changes[].expected_digest`，不要复述完整旧值；否则 `expected` 直接原样回填该字段读取时的 `current_value`（普通字段）或 `platform_value.stored_value`（`_ios` 平台字段的严格存储值，不能用 `resolved_value`）。除显式 `force=true` 外，两者至少存在一个。
- `title_ios`、`description_ios`、`icon_ios` 是可选覆盖；空值表示继承 Android。清空 `_ios` 字段恢复继承时，`expected` 使用该 iOS 字段的严格存储值，不能使用继承后的展示值。
- `category` 是可编辑的游戏类型；C 端商店标签当前不能通过本 skill 或 CLI 修改。`age_grade`、`apk_package_name`、`mini_game_play_enabled`、`apk_supported_languages` 也不能作为普通字段保存。
...
- H5 使用 `slot:"main"` + `package.type:"h5"`；ID 取 H5 版本 ID，`select-package` 的 package 严格只传 `type` 和 `id`，不得带入返回项中的 `h5Package`。H5 元数据仅用于本地 pending state / 提审 override。读回 `h5_info=null` 不代表未绑定。
