(function () {
  ...
  var MESSAGES = {
    en: {
      ...
      consoleHint: 'Opens a local service; does not build or install a game runtime. Closing the page does not stop the service; stop it in the console. Media switches apply to Agent tools, not manual console actions.',
    ...
    ja: {
      ...
      consoleHint: 'ローカルサービスを開きます。ビルドやゲームランタイムのインストールは行いません。ページを閉じても停止しません。サービスはコンソールで停止してください。メディア設定は Agent ツールのみ対象で、手動操作には適用されません。',
