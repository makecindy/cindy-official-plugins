...
paths, or another session's directory as tool arguments. This directory boundary
does not prove Git checkout identity or sandbox Xcode build scripts. Build only
a trusted project: its scripts run as the current macOS user, may read or modify
files outside the project, and return build output to the Agent.
...
Prefer a worktree-relative path. An absolute path is allowed only when its
resolved target is still inside that worktree; `..` or symlink traversal must
...
1. Call `build_app` on the current Host route. On `AMBIGUOUS_XCODE_PROJECT`,
   select the intended in-worktree container or shared scheme from the returned
