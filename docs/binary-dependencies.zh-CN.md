...
- 可选的 `encoding: "brotli"` 使用仓库固定的 Node 内置 Brotli 编码器
  ...
  压缩参数属于打包器，不属于插件契约。公共依赖收集器按声明的 `sha256` 校验上游下载；
  安装包完整性校验由公共分发与客户端安装流程负责。插件不应在运行时对随包二进制
  重复增加哈希校验，无论校验对象是压缩文件还是解码后的程序。插件适配代码负责选择
  平台、按需解码和调用程序；调整压缩参数不应要求修改插件校验数据。
  这不取消公共下载校验、安装包校验或运行时解码限额。
...
（网络传输及 SHA-256 校验）、`extract`、`brotli`、`zip_source`、`zip_dependency`、
