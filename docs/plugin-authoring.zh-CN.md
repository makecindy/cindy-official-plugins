...
3. 按贡献指南运行仓库门禁及相关插件测试。可直接使用 PR CI 的验证产物，
   [依赖接入指南](binary-dependencies.zh-CN.md)说明本地开发、迁移与下载步骤。
   若选择本地复现 CI，官方打包脚本
   `.github/scripts/package-plugin.sh <目录> <输出.cindy>` 从已提交的 **HEAD** 归档
   该插件并补入固定的仓库法律文件，不会包含插件目录中的未提交修改。
   [声明的二进制依赖](binary-dependencies.zh-CN.md) 也由该脚本收集进同一个包
   （本地复现依赖收集时需要 Python 3.11+，不开放插件构建钩子）。Forge 本地打包则
   须先准备对应依赖输出，不会下载依赖。禁止递归压缩
