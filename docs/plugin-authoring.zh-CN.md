...
第三方 CLI 或 stdio MCP 随插件提供，由插件自己的 Node 适配代码通过现有
`cindy.node.request` 接入，不为具体 CLI 新增 Cindy 执行接口。固定上游版本、校验和及
许可证，由适配代码而非 prompt 限制命令、凭据转交、文件路径及副作用失败语义。
Node 是可信的当前用户级代码，不是系统沙箱。

复用插件 OAuth 账号时，`node.secretBindings` 可用 `oauthSecret` 引用本插件的
`network.secrets` OAuth key，原有方法与入口白名单仍生效。每次请求的 `authAccount`
选择不透明账号 ID，省略时使用本插件默认账号。Cindy 刷新令牌，只在
`message.cindy.secrets[binding.key]` 注入短期 access token。不下放 refresh token，
不写全局环境、不落令牌文件、不提供返回凭据的工具。缺失、失效或错误账号直接失败，
不回退到静态凭据。不带 `oauthSecret` 的旧静态绑定行为不变。新能力需要对应客户端，
...
账号 ID，昵称不构成身份或授权。实现与回归见插件自己的
[账号元数据模块](../scripts/google-workspace/account-metadata.js)和
[测试](../.tests/google-account-metadata.test.mjs)。
...
- Agent 应根据这个插件依赖的清单格式、Host 接口及正式稳定版或 Beta 版发布证据确定
...
4. 在满足真实最低版本的正式稳定版或 Beta 版 Cindy 上安装最终包，实际调用核心工具、确认失败
