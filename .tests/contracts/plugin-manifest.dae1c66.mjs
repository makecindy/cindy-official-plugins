...
// source sha256: dae1c66de845889a98745ea876be48e540fdc8fa68084d526495135b82d2a8e4
...
export const GHOST_MANIFEST_SCHEMA_VERSION = 3;
...
const V3_BOOLEAN_TO_LEGACY_SLOT = {
    notify: 'notify',
    badge: 'badge',
    confirm: 'confirm',
    fs: 'fs',
...
function prepareGhostManifestForValidation(value) {
    ...
    for (const [field, slot] of V3_DECLARATION_TO_LEGACY_SLOT) {
        if (value[field] !== undefined)
            syntheticSlots.push(slot);
    }
    for (const field of V3_BOOLEAN_CAPABILITY_FIELDS) {
        if (value[field] === true)
            syntheticSlots.push(V3_BOOLEAN_TO_LEGACY_SLOT[field]);
    }
    ...
    return {
        ...
        prepared: {
            ...
            v3BaseCard,
            v3BaseAgent,
            unknownV3Fields: Object.fromEntries(Object.entries(value).filter(([key]) => key === 'model' || !GHOST_MANIFEST_KNOWN_TOP_LEVEL_FIELDS.has(key))),
        },
    };
}
...
function dropEmptyLegacyCapabilitySlots(value, slots) {
    return slots.filter((slot) => {
        ...
        if (normalized === 'panel')
            return value.panel !== undefined;
        if (normalized === 'cindy')
            return value.cindy !== undefined || value.model !== undefined;
 ...
 */
export function validateGhostManifest(value) {
    ...
    if (raw.mainView !== undefined) {
        ...
        const unknownMainViewField = Object.keys(raw.mainView).find((field) => field !== 'html' && field !== 'title' && field !== 'icon');
        ...
        if (raw.mainView.title !== undefined &&
        ...
        if (raw.mainView.icon !== undefined &&
            !GHOST_MAIN_VIEW_ICONS.includes(raw.mainView.icon)) {
            return {
                ok: false,
        ...
        mainView = {
            ...
            ...(raw.mainView.icon !== undefined ? { icon: raw.mainView.icon } : {}),
    ...
    const cindyRaw = raw.cindy !== undefined ? raw.cindy : prepared.schemaVersion === 2 ? raw.model : undefined;
    let cindy;
            ...
            for (const s of n.secrets) {
                ...
                if (s.oauth !== undefined) {
                    ...
                    if (oa.clientIdAlternatives !== undefined) {
                        ...
                        if (!Array.isArray(oa.clientIdAlternatives) ||
                    ...
                    if (oa.identity !== undefined) {
                        ...
                        if (idn.avatarPath !== undefined) {
                            if (typeof idn.avatarPath !== 'string' ||
    ...
    if (raw.agent !== undefined) {
        ...
        const unknownAgentField = Object.keys(agentRaw).find((key) => key !== 'background' && key !== 'errand' && key !== 'schedule');
    ...
    if (raw.setup !== undefined) {
        ...
        if (!Array.isArray(setupRaw.requires) ||
        ...
        const secretByKey = new Map([
            ...(network?.secrets ?? []).map((secret) => [
                secret.key,
                {
                    hostDerivedSource: secret.source === 'login-email' ||
            ...
            ...(node?.secretBindings ?? []).map((secret) => [secret.key, { hostDerivedSource: null }]),
        ]);
        ...
        for (const group of setupRaw.requires) {
            ...
            for (const requirement of group.anyOf) {
                ...
                if (typeof requirement === 'string') {
                    ...
                    if (typeof requirement.kv === 'string' &&
