...
test('discovery continues past mismatched and unpaired bridges without opening tabs',async()=>{
  ...
  const chrome={runtime:{id:'fixture',getManifest:()=>({version:'0.3.5'}),getURL:()=> 'chrome-extension://fixture/',onStartup:noop,onInstalled:noop,onMessage:noop},
  ...
  const context={chrome,MyBrowserNetwork:{create:()=>({})},MyBrowserPolicy:{},navigator:{userAgent:'Chrome'},URL,AbortController,setTimeout,clearTimeout,crypto:globalThis.crypto,
    fetch:async url=>{const port=Number(new URL(url).port);ports.push(port);return {ok:true,json:async()=>({plugin:'my-browser',protocol:3,extensionId:'fixture',version:port===18810?'0.2.0':'0.3.5',pairing_required:port===18811,session:'fixture-session'})};}};
