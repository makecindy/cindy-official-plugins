...
const pluginRoot = new URL('../cindy-gitlab/', import.meta.url);
...
test('GitLab settings follow the host locale with English fallback', () => {
...
test('GitLab localizes multi-instance controls, validation, and runtime notifications', () => {
  for (const key of [
    'noInstances',
    'setDefault',
    'portUnsupported',
  ...
  assert.match(mainSource, /settingsMessage\(m\.locale, 'failed'\)/);
  assert.match(mainSource, /settingsMessage\(m\.locale, 'success'\)/);
});
