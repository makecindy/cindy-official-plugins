...
3. 用户抱怨"明明连上了却搜不出来" → `x_status` 传 `probe: true` 实测。
   最常见的原因是 **403 订阅档位门禁**:xAI 只对部分订阅档放行 OAuth 推理面。
   这不是登录失败,**别让用户反复重登**,让他配一个 xAI API key 兜底。
   (订阅路由本身跑 `x_search` 是通的——已实测走通;403 是账号档位问题,不是通道问题。)
