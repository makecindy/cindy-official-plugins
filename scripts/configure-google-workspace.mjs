...
for (const [id, [prefix, oauthKey]] of Object.entries(plugins)) {
  ...
  const run = { name: prefix + '_run', description: 'Execute a discovered gog command with a connected account. Prefer this for new tasks, including file and attachment operations. Writes require explicit user intent; never retry an unknown write outcome automatically.', parameters: { type: 'object', properties: {
  ...
  manifest.tools = [schema, run, ...manifest.tools.filter((tool) => tool.name !== schema.name && tool.name !== run.name)];
  ...
  main = main.split(marker)[0];
  const guard = "  if (message && (message.tool === '" + schema.name + "' || message.tool === '" + run.name + "')) return;\n";
  if (!main.includes(guard)) main = main.replace('cindy.onHostMessage(async function (message) {\n', 'cindy.onHostMessage(async function (message) {\n' + guard);
  await fs.writeFile(mainPath, main + marker + accountMetadata + '\n' + (await fs.readFile(path.join(source, 'main-gog.js'), 'utf8')).replaceAll('__TOOL_PREFIX__', prefix));
