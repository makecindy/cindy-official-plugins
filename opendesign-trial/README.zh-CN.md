# OpenDesign for Cindy — 本地试用 0.4.23
...
0.4.21：稿件 iframe（含缩略图及演示预览）不再授予 allow-downloads。稿件中的 data/blob 或本项目下载链接向父编辑器提出文件请求，只有真实点击“保存文件”才授权下载；界面显示文件名和字节数，限制 12 MiB，可取消。父页面原有导出入口保留。实机验收已确认；按用户范围决定，WebRTC 隔离不在本插件改动中处理，不宣称风险已修复。
...
批注与 Inspect 选点改由父页面真实指针手势进行一次性授权，稿件消息不能清空或换绑未保存批注。每次目标广播最多扫描 1,000 个节点、返回 500 个可见候选，父页面拒绝超限数组；超出广播预算的元素仍可直接点击选中。

首次创建先保留可恢复的初始化项目；初次写入失败后可在原会话、原目录重试，写入成功才标记为已创建。若写入已完成但回执或最终状态保存失败，恢复时保留已有稿件，不覆盖内容。
