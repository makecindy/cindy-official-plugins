...
export async function fetchProjectPreviewBaseHref(
  ...
  return {href:(window as any).OD.previewBase + name.split('/').slice(0,-1).map(encodeURIComponent).join('/') + (name.includes('/')?'/':''),expiresAt:Date.now()+3600000};
...
export async function fetchProjectFileText(
  ...
  const url = projectFileUrl(projectId, name, options?.workspaceContext) + '?source=1';
...
export function projectRawUrl(
  ...
  const safePath = filePath
    ...
    .map((seg) => encodeURIComponent(seg))
    .join('/');
  return (window as any).OD.previewBase + safePath;
}
